...
object MultiMapStatisticTest {
  type logParserStatResult =
    mutable.Map[String, mutable.Map[String, mutable.Map[Int, Int]]]
  ...
  def main(args: Array[String]) {
    ...
    val records: List[logParserStatRecord] = List(
        ("TargetInfoDataObject", "spv", 544, 3),
        ("TargetInfoDataObject", "spv", 544, 5),
        ("WdPvDataObject", "spv", 433, 5),
        ("TargetInfoDataObject", "pv", 544, 3),
        ("TargetInfoDataObject", "spv", 722, 2))
  ...
  def showMultiMap(dataTypeMap: logParserStatResult) = {
    for {
      ...
      println(dataType,
              gsCommand,
              profileId,
              dataTypeMap(dataType)(gsCommand)(profileId))
  }

  def addAccumulator(
      dataTypeMap: logParserStatResult, record: logParserStatRecord) = {
    val (dataType, gsCommand, profileId, countNum) = record
    // val gsCommandMap = dataTypeMap(dataType)
    // val gsCommandMap = dataTypeMap(dataType)
    val gsCommandMap = dataTypeMap.getOrElse(
        dataType, new mutable.HashMap[String, mutable.Map[Int, Int]]())
    val profileIdMap =
      gsCommandMap.getOrElse(gsCommand, new mutable.HashMap[Int, Int]())
