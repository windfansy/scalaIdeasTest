...
def _execute_impala_sql_(sql, target_db="wd3", extra_cmd=None):
    sql = "Use {0};".format(target_db) + sql
    print sql + " ------- will be executed"
    if extra_cmd is None:
        cmd = '''impala-shell -q'''.split(' ') + [sql]
        subprocess.call(cmd)
    else:
        cmd = '''impala-shell -q'''.split(' ') + [sql]
        # print extra_cmd.split(' ')
        extra_cmd_manual = ['awk', '-F', '\|', 'NR>3{print $2}/STORED AS PARQUET/{print \";\";exit}']
        p1 = subprocess.Popen(cmd, stdout=subprocess.PIPE)
        p2 = subprocess.Popen(extra_cmd_manual, stdin=p1.stdout)
        p2.communicate()
...
def describe_tables(all_table=True):
    if all_table:
        for tb in PARTITION_PROFILE_GROUP_TABLES + PARTITION_DATE_TABLES + NON_PARTITION_TABLES + PARTITION_PROFILEID_TABLES:
            ...
            _execute_impala_sql_(desc_sql)


def show_create_tables(all_table=True):
    if all_table:
        for tb in PARTITION_PROFILE_GROUP_TABLES + PARTITION_DATE_TABLES + NON_PARTITION_TABLES + PARTITION_PROFILEID_TABLES:
            tb = _map_table_(tb)
            desc_sql = _show_create_table_sql_(tb)
            _execute_impala_sql_(desc_sql, extra_cmd="awk -F '\\|' 'NR>3{print $2}/STORED AS PARQUET/{print \";\";exit}'")


def drop_tables(all_table=True):
    if all_table:
        for tb in PARTITION_PROFILE_GROUP_TABLES + PARTITION_DATE_TABLES + NON_PARTITION_TABLES + PARTITION_PROFILEID_TABLES:
            tb = _map_table_(tb)
            drop_sql = _drop_table_sql_(tb)
            _execute_impala_sql_(drop_sql)
